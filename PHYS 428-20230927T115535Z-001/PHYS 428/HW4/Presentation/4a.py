...
def gssSdl(A,b):
    #takes 3x3 matrix A and 3x1 b (as numpy arrays) in Ax=b, solves x by Gauss-Seidel iteration with initial geuss x = 0
    #and convergence tolerance of 1e-7. Returns solution x as a numpy array
...
def gSsor(A,b,w):
    #takes 3x3 matrix A and 3x1 b (as numpy arrays) in Ax=b, solves x by Gauss-Seidel iteration with SOR, opt. parameter w,
    #with initial geuss x = 0 and convergence tolerance of 1e-7. Returns solution x as a numpy array
    ...
    while vectErr > 1e-7:
        ...
        xk1 = np.zeros((3,1))
        #crudely solve for xk at the kth iteration
        xk1[0,0] = xk0[0,0] - w*(xk0[0,0] + (1/A[0,0])*((A[0,1]*xk0[1,0]) + A[0,2]*xk0[2,0] - b[0,0]))
        xk1[1,0] = xk0[1,0] - w*(xk0[1,0] + (1/A[1,1])*((A[1,0]*xk1[0,0]) + A[1,2]*xk0[2,0] - b[1,0]))
        xk1[2,0] = xk0[2,0] - w*(xk0[2,0] + (1/A[2,2])*((A[2,0]*xk1[0,0]) + A[2,1]*xk1[1,0] - b[2,0]))        
        #compute the error between iterations as the vector component with the greatest error
        vectErr = LA.norm((xk1-xk0),(np.inf))
        #set xk for the next iteration
        xk0 = xk1
...
w = 2/(1+((7/8)**(0.5)))
print('The solution x from Gauss-Seidel with SOR is approximately...')
print(gSsor(A,b,w))
